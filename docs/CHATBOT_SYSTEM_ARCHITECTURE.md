...
| Camera Troubleshooting | Diagnose offline, delayed, or low-FPS cameras | Camera API, service health, recent errors | Phase 2 — orange. **Shipped 2026-09-16 in narrowed form** (Section 9k): deterministic diagnoses from real camera status fields + a live frame-arrival check; last-seen time, backend errors, measured FPS and pipeline health are not exposed by the deployed backend and are reported as unavailable, not inferred. |
...
| Incident Timeline | Correlate image-backed events within a bounded scope | Alert search, image evidence; cross-camera/video correlation deferred | Phase 2 (Timeline & Sequence) — orange. **Shipped 2026-09-16 in narrowed form** (Section 9k): chronological alert episodes per camera from evidence capture timestamps — the only time signal the deployed backend records; event/processing/notification times are reported as unavailable. |
...
### 9k. Phase 2 Begins: Camera Troubleshooting and Timeline & Sequence (2026-09-16)

The first two Phase 2 agents, chosen after probing what the real backends actually expose rather than what Section 1.2's table assumed. Both shipped in a deliberately narrower form than their catalog cards describe, and both say so structurally (a `not_available` list in every tool result) rather than only in prose.

**What the probe found first.** The ServiceAPI (`API_SERVICE`, :4000) turns out to already expose the capabilities two other Phase 2 agents need — `POST /ImageAnalysis` (→ Image & Anomaly Analysis) and `POST /AlertReportAnalyzer` (→ Alert Summary), with exact contracts readable from its `/openapi.json` and valid values (`model_option: "GPT 4o"`, `lang_option: "eng"`) confirmed from the frontend's own calls. The "minor backend change" Section 0.1a anticipated for those two already exists. **But every one of those endpoints currently fails:** the `API_SERVICE` container has no MongoDB configuration at all (its only env vars are `PATH` and `TZ`), so its Mongo host resolves to the literal string `none` and each call dies after a 30s timeout — the same class of deployment misconfiguration Section 9e found on `node_backend`. It's a compiled PyInstaller binary this repo can't inspect or fix; unblocking Alert Summary and Image & Anomaly Analysis needs whoever owns that deployment to supply its database env vars. **Separately, and more urgent than any chatbot work:** the same port exposes unauthenticated, zero-argument `POST /DockerClean` (a `docker system prune`), `POST /Notifications` (restarts every running camera), and `GET /rtsplinks.json` (plaintext RTSP credentials — the exact file Section 5.6 flagged as committed to the repo, now also served over HTTP). None were invoked; all three were read from the schema. Network access to :4000 should be restricted regardless of anything in this document.

**Camera Troubleshooting** (`app/tools_troubleshooting.py`, agent `camera_troubleshooting`). The catalog wanted last-seen time, RTSP health, service state and recent backend errors; probed live, none of that exists (`/api/myAlert`, `/api/investigation`, `/api/alertHistory` and the per-camera `recentAlert` variant all 404). Built from what is real: the camera record's `Status`/`Surveillance_Status`/`Live`/`Active`/`FPS` fields — each with a meaning already verified in source by `tools_errors.py`'s glossary — plus `app/socket_probe.py`'s live frame-arrival check as the RTSP-health proxy. Diagnosis is a deterministic rule table (observed signal, likely cause, confidence, next step), not LLM reasoning, for the same reason `extract_analytics` is: a wrong "likely cause" sends an operator to fix the wrong thing. Rule precedence matters and is tested — an operator-chosen state (surveillance stopped, feed paused) explains a missing feed over the generic "no frames" rule, and a camera whose `Status` still reads `creating` while demonstrably streaming (true of all three real cameras today) is reported as healthy with a stale label, not as a fault. `FPS` is reported as the *configured* rate, never a measured one — nothing in this API measures frame rate.

**Timeline & Sequence** (`app/tools_timeline.py`, agent `timeline`). The catalog wanted event, evidence, processing and notification timestamps in one schema; the deployed backend records exactly one — the capture time embedded in each alert image's filename. So this is an honest *evidence* timeline, labelled `timestamp_kind: evidence_capture_time`, and the agent is forbidden from calling it anything else. Frames are grouped into per-camera episodes (consecutive frames within `gap_minutes`) with start, end, duration, frame count and the quiet gap since that camera's previous episode; ordering and time arithmetic are computed deterministically. Episodes, not frames, are the returned unit: a real 24-hour window today held 4,675 frames across two cameras — returned raw, that would blow past `compact_results_json`'s cap and silently truncate a camera out of the answer (the 2026-09-04 insights failure). The first live run still came in at 10.6k of the 12k cap because each episode carried a sample image URL the agent is forbidden to state anyway; dropping it brought a 40-episode result to 6.7k.

**A real reliability finding, fixed before deploy.** The first live `diagnose_cameras` run reported two healthy, actively streaming cameras as "no frames arriving" — which would have become a false "check your RTSP connection" diagnosis. Measured: the 1.5s probe window used since 9j was flaky (2 of 3 back-to-back runs missed a 3-FPS camera — frame delivery over the socket is bursty), while 3s and 6s were correct on every run. The default (`LIVE_PROBE_TIMEOUT_SECONDS`) is now 4s for Live Monitoring, and the diagnostic tool uses its own 6s window (`DIAGNOSTIC_PROBE_TIMEOUT_SECONDS`) — a few seconds of latency is affordable; a false "broken camera" diagnosis isn't.

**Router.** Two disambiguation rules added (`app/router.py`): a fault/"why"/"not working" framing is `camera_troubleshooting`, distinct from `live_monitoring`'s plain status snapshot and `camera_operations`' settings; a chronological framing ("timeline", "sequence", "before/after/between", "gaps") is `timeline`, distinct from `alert_investigation`'s plain alert list. Five regression cases added, including one guarding that "show me the recent alerts for cam3" is *not* pulled into `timeline`.

**Verified live:** 45/45 pytest cases (12 new rule-precedence cases, 10 new timeline cases), 15/15 live router regression, and both agents end-to-end through `/v1/chat/stream` — the troubleshooting answer correctly reported cam3's stale `creating` label as a healthy, streaming camera; the timeline answer narrated a 3-hour cam3 window as evidence-capture episodes with real gap figures. Frontend tiles for both agents shipped in `ChatbotWidget.jsx` and confirmed present in the served bundle on :8080. Also fixed in passing: the SSE `tool_result` event never forwarded the `latency_ms` added in 9j.

### 9l. ServiceAPI Blocker Report — root cause settled from the deployed binary (2026-09-16)

Follow-up to 9k's ServiceAPI finding, written up for the deployment owner as `development-plan/ServiceAPI_Blocker_Report_2026-09-16.pdf`. The facts below were established from the running system and the deployed binaries themselves (read-only: `docker inspect`/`logs`, the ServiceAPI's PyInstaller archive index and bytecode string constants, the Node backend's `pkg` bundle strings), not from the upstream source, which was not yet accessible.

**Root cause, exactly.** The `service_api` block in `/home/algo6/Desktop/Algo6-Aksha/docker-compose.yml` has no `environment:` and no `env_file:`, and the `aksha_service:11022025-1` image ships only the compiled binary — no `prod.json`, no `.env`. The entry script reads its Mongo URI from `os.getenv("mongodb_conn_string_prod")` into `pymongo.MongoClient(..., directConnection=True)`; unset, the host becomes `none` and every DB-backed route dies after pymongo's 30 s server-selection timeout. `/AlertReportAnalyzer` and `/ImageAnalysis` need the DB only to read `Resource.isGenAIFeatures` (a feature flag) before calling the Azure OpenAI modules, which in turn `load_dotenv(".env")` for `AZURE_OPENAI_API_KEY`/`AZURE_OPENAI_ENDPOINT`/`AZURE_API_VERSION`/`AZURE_DEPLOYMENT_GPT_4_TURBO` — also absent. So unblocking Alert Summary and Image & Anomaly Analysis (Section 9k) is three deployment changes: the `mongodb_conn_string_prod` variable (same URI as `node_backend`'s `DATABASE`), a mounted `/Aksha_Pipeline/.env` with the four Azure settings, and the `isGenAIFeatures` flag. `/Insight` reads neither and works.

**Who actually calls the ServiceAPI.** `node_backend` — its bundled source contains `http://API_SERVICE:4000/DockerClean`, `/Insight`, `/Notifications` and `/Surveillance`; the nightly 17:00Z `POST /DockerClean` (a `docker system prune -f`) is the backend's housekeeping call. The 9k-era attribution of that call to `cam1-Gate-Camera` was an IP-reuse artefact (all containers were restarted 04:45Z on 2026-09-16 and IPs reassigned); the camera images contain no reference to port 4000. Consequence for remediation: binding `:4000` to `127.0.0.1` breaks nothing internal, because the backend addresses the service by container alias.

**Exposure, upgraded from 9k.** Beyond the unauthenticated zero-argument `/DockerClean` (`docker system prune -f`) and `/Notifications` (`docker restart` of every camera) and the credential-serving `/rtsplinks.json` (also served, unauthenticated, by the Node API on :5000), the entry script's constants show `/Surveillance` composing a `docker run -d --restart unless-stopped --env ENVIRONMENT="prod.json" --env CAMERA_NAME=… --env RTSP_LINK="…" … --volumes-from API_SERVICE` command string from request fields and executing it via `os.system` as root with the host Docker socket mounted read-write — an unauthenticated command-injection surface, rated critical in the report pending source review (not exploited or tested). CORS reflects any origin with credentials. The report's remediation order: bind to loopback and add a `DOCKER-USER` rule (immediate), rotate RTSP credentials and gate the Node route (immediate), proxy only `/Surveillance` and `/Insight` through the frontend nginx with auth (soon), hardened image build (soon), docker-socket proxy (later).

**Method note.** A multi-agent adversarial verification pass was attempted and did not run (account spend limit; 11 of 12 agents failed to start), so every claim in the report carries an explicit evidence label (Observed / Code / Documented / Inferred) instead. Nothing was invoked on the destructive routes; nothing was modified; all credentials encountered were redacted at the point of reading.
